...
abstract class BaseMedia
{
    ...
    public function rotate($degree)
    {
        ...
        $imagick = new \Imagick($this->getAbsolutePath());
        $imagick->rotateimage(new \ImagickPixel(), $degree);
        $this->setName(sha1(uniqid(mt_rand(), true)) . '.jpeg');
        $this->setPath($this->getWebPath());
        $imagick->writeimage($this->getAbsolutePath());
        ...
        if ($w > 578 || $h > 578) {
            $imagick->adaptiveResizeImage(578, 578, 1);
        }
        $imagick->writeimage(str_replace('original', 'resize', $this->getAbsolutePath()));

        $imagick->cropThumbnailImage(140, 140);
        $imagick->writeimage(str_replace('original', 'thumb', $this->getAbsolutePath()));
    ...
    public function crop($coords)
    {
        ...
        $original = new \Imagick($this->getAbsolutePath());

        //$resize = new \Imagick(str_replace('original', 'resize', $this->getAbsolutePath()));
        //$ratioX = $coords['x'] / $resize->getimagewidth();
        //$ratioY = $coords['y'] / $resize->getimageheight();
        //$x = $ratioX * $original->getimagewidth();
        //$y = $ratioY * $original->getimageheight();
        //$ratioWidth = $coords['w'] / $resize->getimagewidth();
        //$ratioHeight = $coords['w'] / $resize->getimageheight();
        //$width = $ratioWidth * $original->getimagewidth();
        //$height = $ratioHeight * $original->getimageheight();

        $original->cropimage($coords['w'], $coords['h'], $coords['x'], $coords['y']);

        $this->setName(sha1(uniqid(mt_rand(), true)) . '.jpeg');
        $this->setPath($this->getWebPath());
        $original->writeimage($this->getAbsolutePath());

        $d = $original->getImageGeometry();
        $w = $d['width'];
        $h = $d['height'];

        if ($w > 578 || $h > 578) {
            $original->adaptiveResizeImage(578, 578, 1);
        }
        $original->writeimage(str_replace('original', 'resize', $this->getAbsolutePath()));

        $original->cropThumbnailImage(140, 140);
        $original->writeimage(str_replace('original', 'thumb', $this->getAbsolutePath()));
    ...
    public function upload()
    {
        ...
        if ($this->type === self::TYPE_IMAGE) {
            ...
            $image->writeimage($pathNoExtension . ".jpeg");
