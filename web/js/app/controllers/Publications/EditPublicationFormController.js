angular.module('app').controller('EditPublicationFormController', ['$scope', '$http', '$modal', '$timeout', '$filter', '$location', '$facebook', '$maputils', 'factory:Publication', function ($xscope, $http, $modal, $timeout, $filter, $location, $facebook, $maputils, $factory) {
    ...
    $xscope.openObservation = function (data, edit) {
        ...
        modalInstance.result.then(function (params) {
            if (params != null) {
                $xscope.$emit('npevent-publication/update', params.publication);
            } else {
                $xscope.$emit('npevent-publication/update', data.publication);
            }
            $xscope.resetForm();
        });
    };
    ...
    $xscope.cancelEditing = function () {
        ...
        $(".publications").before($("#edit-publication"));
        $xscope.$emit('npevent-publication/update', $xscope.publication);
    };
    $xscope.updatePublication = function ($event) {
        ...
        $factory.updateMedia($xscope.publication)
            .success(function (response) {
                ...
                $(".publications").before($("#edit-publication"));
                $xscope.show = false;
                $xscope.$emit('finishPublication', $xscope.publication);
                if ($xscope.specification) {
                    var aGroups = [];
                    var aSavedGroups = [];
                    ...
                    $xscope.$emit('npevent-publication/update', response.publication);
                    $xscope.resetForm();
                }
            })
    ...
    $xscope.toggleGeolocation = function () {
        ...
        if ($xscope.publication.geolocation.active) {
            if ($xscope.marker) {
                $xscope.marker.setMap(null);
            }
            $xscope.marker = new google.maps.Marker({
                map      : map,
                draggable: true,
                icon     : window.location.protocol + '//' + window.location.hostname + '/img/map/map_icon_loc.png',
            });
            //map.setCenter(new google.maps.LatLng($xscope.publication.geolocation.latitude, $xscope.publication.geolocation.longitude));
            if ($xscope.publication.geolocation.latitude && $xscope.publication.geolocation.longitude) {
                var myLatLng = new google.maps.LatLng($xscope.publication.geolocation.latitude, $xscope.publication.geolocation.longitude);
                $xscope.savedGeolocation = myLatLng;
            }
            google.maps.event.addListener($xscope.marker, 'dragend', $xscope.markerDragged);
