angular.module('app').controller('EditMediaPublicationFormController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', '$filter', '$location', '$facebook', '$maputils', 'factory:Publication', function ($rootScope, $escope, $http, $modal, $timeout, $filter, $location, $facebook, $maputils, $factory) {
    ...
    $escope.$on('editMediaPublicationForm', function (event, data) {
        ...
        $(".modal-publication-text.pull-right").css("width", "600px");
    });
    $escope.cancelEditing = function () {
        $escope.show = false;
        $escope.publication = angular.copy($escope.backupPublication);
        $escope.publication.editing = false;
        $escope.publication.show = true;
        $("#corps-container").before($("#edit-media-publication"));
        $rootScope.$broadcast('npevent-publication/cancel', $escope.publication);
        $rootScope.$broadcast('npevent-publication/update', $escope.publication);
    ...
    $escope.updatePublication = function ($event) {
        ...
        $factory.updateMedia($escope.publication)
            .success(function (data, status, headers, config) {
                ...
                $escope.resetForm();
                $("#corps-container").before($("#edit-media-publication"));
                $rootScope.$broadcast('finishPublication', $escope.publication);
                if (status === 201) {
                    $rootScope.$broadcast('npevent-publication/update', data.publication);
    ...
    $escope.toggleGeolocation = function () {
        ...
        if ($escope.publication.geolocation.active) {
            setTimeout(function () {
                initMap($escope.publication.geolocation.latitude, $escope.publication.geolocation.longitude, "_edit");
            }, 1000)
            $escope.map.loading = false;
        } else {
