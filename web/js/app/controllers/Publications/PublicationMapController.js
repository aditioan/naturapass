...
angular.module('app').controller('PublicationController', ['$rootScope', '$scope', '$http', '$filter', '$modal', '$timeout', '$location', '$facebook', 'factory:Publication', 'factory:PublicationComment', function ($rootScope, $scope, $http, $filter, $modal, $timeout, $location, $facebook, $factoryPublication, $factoryPublicationComment) {
        $scope.loaded = false;
        $scope.sharings = [];
        ...
        $scope.$on('npevent-publication/cancel', function (event, data) {
        $scope.publication=data;
        $scope.publication.editing = false;
        })
        $scope.startEditingPublication = function ($event) {
            $scope.$emit('editPublication', $scope.publication);
            $rootScope.$broadcast('editPublication', $scope.publication);
        ...
        .controller('ModalShowObservationController', ['$scope', '$controller', '$timeout', '$q', '$maputils', '$modalInstance', 'publication', function ($scope, $controller, $timeout, $q, $maputils, $instance, publication) {
                $scope.publication = publication;
...
function initMap(lat, lng, type) {
    ...
    map = new google.maps.Map(document.getElementById('geolocation'+type+'_map_publication'), {
        zoom: 15,
        center: myLatLng
    });
    ...
    var marker = new google.maps.Marker({
        ...
        map: map,
        draggable: true,
        icon: '/img/map/map_icon_loc.png'
    });
    google.maps.event.addListener(marker, 'dragend', function (event) {
        $("#publication"+type+"_geolocation_latitude").val(event.latLng.lat());
        $("#publication"+type+"_geolocation_latitude").trigger('input');
        $("#publication"+type+"_geolocation_longitude").val(event.latLng.lng()); 
        $("#publication"+type+"_geolocation_longitude").trigger('input');
        geocoder.geocode({'location': {lat: event.latLng.lat(), lng: event.latLng.lng()}}, function (results, status) {
            if (status === google.maps.GeocoderStatus.OK) {
                if (results[0]) {
                    $("#publication"+type+"_geolocation_address").val(results[0].formatted_address);
                    $("#publication"+type+"_geolocation_address").trigger('input');
                } else {
