angular.module('app').controller('FdcListingController', ['$scope', '$http', '$filter', '$location', function ($scope, $http, $filter, $location) {
    $scope.tables = [];
    $scope.tfoot = {};
    $scope.showTfoot = [];
    $scope.showPanelCard = [];
    ...
    $scope.exportExcel = function (card) {
        window.open($filter('route')('api_backoffice_get_observations_excel', {card: card}), "_blank", null);
    };
    ...
    $scope.init = function () {
        $http.get($filter('route')('api_backoffice_get_observations'))
