 ...
 */
angular.module('app').controller('FdcSearchController', ['$scope', '$http', '$filter', '$location', function ($scope, $http, $filter, $location) {
    $scope.selected = {
        groups    : [],
        users     : [],
        localities: [],
    ...
    $scope.initFdcSearch = function () {
        ...
        $http.get($filter('route')('api_backoffice_get_categories'))
            .success(function (data) {
                $scope.dataTree = data.tree;
                $scope.dataTree = $scope.dataTree.filter(function (item) {
                    return item.id != 643 && item.id != 828; // ignore FDC76 & FDC77
                });
            })
            .error(function (data) {
            });
